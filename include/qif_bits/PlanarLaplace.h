...
namespace mechanisms {
	...
	planar_laplace_grid(uint width, uint height, eT step, eT epsilon) {
		...
		Mechanism<eT> mech;
		mech.d = metrics::scale(metrics::grid<eT, Point<eT>>(width), step);
		mech.C = arma::zeros<Mat<eT>>(size, size);

		int cx = width - 1;
		int cy = height - 1;

		Mat<eT> big_matrix = internal::grid_integration(2*width-1, 2*height-1, step, epsilon);
		...
		for(uint input_x = 0; input_x < width; input_x++) {
			for(uint input_y = 0; input_y < height; input_y++) {
				...
				//
				for(int x = -cx; x <= cx; x++) {
					uint output_x = internal::bound(input_x + x, 0, width-1);

					for(int y = -cy; y <= cy; y++) {
						uint output_y = internal::bound(input_y + y, 0, height-1);
