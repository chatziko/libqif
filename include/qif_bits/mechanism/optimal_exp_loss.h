...
namespace mechanism::optimal_exp_loss {

namespace aux {
	// auxiliary, hidden from the outside

	template<typename eT>
	Chan<eT> under_d_privacy_all(
		const Prob<eT>& pi,
		uint n_cols,
	...
	template<typename eT>
	Chan<eT> under_d_privacy_dist(Prob<eT> pi, uint n_cols, Metric<eT, uint> d_priv, Metric<eT, uint> loss) {
	...
	//
	template<typename eT>
	Chan<eT> under_d_privacy_dist_strict(const Prob<eT>& pi, uint n_cols, Metric<eT, uint> d_priv, Metric<eT, uint> loss) {
...
Chan<eT> under_d_privacy(
	...
	if(vars == "all")
		return aux::under_d_privacy_all(pi, n_cols, d_priv, loss, inf);
	else if(vars == "dist")
		return aux::under_d_privacy_dist(pi, n_cols, d_priv, loss);
	else if(vars == "dist_strict")
		return aux::under_d_privacy_dist_strict(pi, n_cols, d_priv, loss);
	else
...
// Same as under_max_g_vulnerability for the identity gain function, but faster to construct the LP.
//
template<typename eT>
Chan<eT> under_max_bayes_vuln(
	const Prob<eT>& pi,
	uint n_cols,
	eT max_vuln,
	Metric<eT, uint> loss,
...
//
template<typename eT>
Chan<eT> under_max_g_vulnerability(
	const Prob<eT>& pi,
	uint n_cols,
	uint n_guesses,
	eT max_vuln,
