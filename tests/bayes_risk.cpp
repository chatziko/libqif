...
TYPED_TEST_P(BayesRiskTest, Vulnerability) {
	...
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::prior(t.dirac_4));
...
TYPED_TEST_P(BayesRiskTest, Post_vulnerability) {
	...
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.unif_2, t.id_2));
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.dirac_2, t.id_2));
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.pi1, t.id_2));

	if(!std::is_same<eT, float>::value) { // precision error on floats
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.unif_10, t.id_10));
	}
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.dirac_10, t.id_10));
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.pi2, t.id_10));

	EXPECT_PRED_FORMAT2(equal2<eT>, eT(9)/10, bayes_risk::posterior(t.unif_10, t.noint_10));
	EXPECT_PRED_FORMAT2(equal2<eT>, 0,        bayes_risk::posterior(t.dirac_10, t.noint_10));
...
TYPED_TEST_P(BayesRiskTest, Mult_capacity) {
	...
	EXPECT_PRED_FORMAT2(equal2<eT>, 1,              bayes_risk::mult_capacity(t.noint_10));
	EXPECT_PRED_FORMAT2(equal2<eT>, 2,              bayes_risk::mult_capacity(t.c1));
