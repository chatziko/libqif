...
xValues = np.arange(1, len(locations)+1, 1)
...
for i in range(len(labels)):
    ...
    CT[i], = axs[0].plot(xValues, c_t, color=l_colors[i], ls='-', marker=l_mark[i], markeredgecolor=l_colors[i])
...
for i in range(len(labels)):
    ...
    UL[i], = axs[1].plot(xValues, ul, color=l_colors[i], ls='-', marker=l_mark[i], markeredgecolor=l_colors[i])
