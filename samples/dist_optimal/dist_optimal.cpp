...
void print_mech(std::string name, chan& C, double eps, prob& pi, Metric<double, uint> d, Metric<double, uint> loss) {
	cout << "\n---- " << name << " ----\n";
//	cout << C;
	cout << "eps deviation: " << (measure::d_privacy::smallest_epsilon(C, d) - eps) << "\n";
...
//	prob pi = load_prior("/home/vagabond/Desktop/ehab_priors/p1.pr");
	...
	chan opt = mechanism::d_privacy::min_loss_given_d(pi, n_outputs, eps * dx, loss);
	print_mech("optimal", opt, eps, pi, dx, loss);

	chan dist_opt = mechanism::d_privacy::min_loss_given_d(pi, n_outputs, eps * dx, loss, "dist");
	print_mech("dist-optimal", dist_opt, eps, pi, dx, loss);

	chan dist_opt_strict = mechanism::d_privacy::min_loss_given_d(pi, n_outputs, eps * dx, loss, "dist_strict");
